...
count_kb = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton('Рассчитать норму каллорий', callback_data='calories'),
...
async def main_menu(message):
    await message.answer("Выберите опцию", reply_markup=count_kb)
...
async def get_buying_list(message):
    for i in range(4):
        all_prod = get_all_products()[i]
        title = all_prod[1]
        description = all_prod[2]
        price = all_prod[3]
